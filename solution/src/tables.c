...
SymbolTable* create_table(int mode) {
  ...
  if (symbTable == NULL) {allocation_failed();}
  symbTable->tbl = malloc(sizeof(Symbol));
  if (symbTable->tbl == NULL) {free(symbTable); allocation_failed();}
  symbTable->cap = 1;
  symbTable->len = 0;
  symbTable->mode = mode;
  return symbTable;
...
int add_to_table(SymbolTable* table, const char* name, uint32_t addr) {
  /* YOUR CODE HERE */
  Symbol *temp = malloc(sizeof(Symbol));                          /*Declares the variable pointers to be inserted*/
  temp->name = malloc(sizeof(name)*4 + 1);
  int i;
  for (i = 0; i < table->len; i++)                                /*Iter over the table to see if there exists the same name*/
  {
    if (strcmp(table->tbl[i].name, name) == 0 && table->mode)
    {
      name_already_exists(name);                              /*Calls name_already_exists and returns -1 if true.*/
      free(temp->name); free(temp);                           /*Frees the memory on the heap before it returns*/
      return -1;
    }
  }
  if (addr % 4 != 0)                                              /*Case not aligned*/
  {
    addr_alignment_incorrect();                                 /*Calls addr_alignment_incorrect and returns -1 if true*/
    free(temp->name); free(temp);                               /*Frees the memory on the heap before it returns*/
    return -1;
  }
  if (table->len >= table->cap)                                   /*If tbl is not long enough, increase it.*/
  {
    table->tbl = (Symbol *) realloc(table->tbl, (table->cap * 2 * sizeof(Symbol)));
    table->cap *= 2;
  ...
  {
    strcpy(temp->name, name);                                   /*Do the same things except for increasing its capacity as above*/
    temp->addr = addr;
    table->tbl [i] = *temp;
    free(temp);
    table->len++;
...
int64_t get_addr_for_symbol(SymbolTable* table, const char* name) {
  ...
  for (i = 0; i < table->len; i++) {                              /*Iter through the whole tbl*/
    if (strcmp(table->tbl[i].name, name) == 0 )                 /*If the name we are looking for exists*/
    {
      return table->tbl[i].addr;                              /*Returns its addr*/
    }
  }
  return -1;                                                      /*Returns -1 if there does not exists the name we are looking for.*/
}
...
void write_table(SymbolTable* table, FILE* output) {
  ...
  int i;
  for (i = 0; i < (*table).len; i++) {                            /*Iter through the whole tbl*/
    write_sym(output, table->tbl[i].addr, table->tbl[i].name);  /*Call write_sym and puts the parameters into it.*/
  }
}
