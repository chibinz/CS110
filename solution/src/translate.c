...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
  ...
  if (strcmp(name, "li") == 0) {
    ...
    if (num_args == 2)
    {
      if (translate_reg(args[0]) == -1)/*valid register*/ return 0;
    ...
    if (num_args == 2)
    {
      if (translate_reg(args[0])==-1 || translate_reg(args[1])==-1)/*valid register*/ return 0;
...
int translate_inst(FILE* output, const char* name, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    else if (strcmp(name, "jal") == 0)  return write_ujtype (0x6F, output, args, num_args, addr, reltbl, symtbl);
...
int write_rtype(uint8_t opcode, uint8_t funct3, uint8_t funct7, FILE* output, char** args, size_t num_args) {
  ...
  /* YOUR CODE HERE */
  if (rd < 0 || rd > 31 || rs1 < 0 || rs1 > 31 || rs2 < 0 || rs2 > 31) return -1;
  /* error checking for register ids */

  instruction = 0;
  /* generate instruction */
  instruction = instruction | (rs1 << 15);                                 /*Shifts 15 bits for rs1 and put it into instruction*/
  instruction = instruction | (rd << 7);                                   /*Shifts 7 bits for rd and put it into instruction*/
  instruction = instruction | (rs2 << 20);                                 /*Shifts 20 bits for rs2 and put it into instruction*/
  instruction = instruction | opcode;                                      /*Shifts 0 bits for opcode and put it into instruction*/
...
int write_stype(uint8_t opcode, uint8_t funct3, FILE* output, char** args, size_t num_args) {
  ...
  imm11_5 = (immediate & 0xfe0) >> 5;                             /* Takes the rest bits of the immediate */
  /* generate instruction */
  /*printf ("Immediate: %lx, imm4_0: %x\n", immediate, imm4_0);*/
  instruction = instruction | (funct3 << 12);
  instruction = instruction | (opcode << 0);                              /*Shifts 0 bits for opcode and puts it into instruction*/
  instruction = instruction | (rs1 << 15);                                 /*Shifts 15 bits for rs1 and puts it into instruction*/
  instruction = instruction | (rs2 << 20);                                 /*Shifts 20 bits for rs2 and puts it into instruction*/
  instruction = instruction | (imm4_0 << 7);                                  /*Shifts 7 bits for immediate[4:0] */
...
int write_sbtype(uint8_t opcode, uint8_t funct3, FILE* output, char** args, size_t num_args,
  ...
  char *name;
  /*Declares before expressions*/
  if (num_args != 3)  return -1;                                          /*Returns -1 if too much or too few args*/
  rs1 = translate_reg(args[0]);                                            /*Translates rs from args[1]*/
  rs2 = translate_reg(args[1]);                                            /*Translates rt from args[1]*/
  name = args[2];
  if (rs1 < 0 || rs2 < 0 || rs1 > 31 || rs2 > 31 || !symtbl)    return -1;    /*If translate_reg or translate_num fails, return -1*/
  L = (int) get_addr_for_symbol(symtbl, name);
  if (L < 0) return  -1;
  if ((L - addr - 4) % 4 != 0)    return -1;
  /*if ((L - addr) % 4 != 0)  return -1;*/
  /*I = (int16_t) (L - addr - 4) / 4;*/
  I = (int16_t) (L - addr);
  instruction = instruction | opcode;                                     /*Shifts 0  bits for opcode and puts it into instruction*/
  instruction = instruction | (rs1 << 15);                                 /*Shifts 15 bits for rs1 and puts it into instruction*/
  instruction = instruction | (rs2 << 20);                                 /*Shifts 20 bits for rs2 and puts it into instruction*/
  instruction = instruction | (funct3 << 12);
  /*I = I & 0xffff;*/
  ...
  imm11 = (I & 0x800) >> 11;
  imm12 = (I & 0x1000) >> 12;
  /*printf ("imm4_1: %x, imm10_5: %x, imm11: %x, imm12: %x\n", imm4_1, imm10_5, imm11, imm12);*/
  /*printf("imm12: %x, imm10_5: %x, imm4_1: %x, imm11: %x\n", imm12, imm10_5, imm4_1, imm11);*/
  instruction = instruction | (imm12 << 31) | (imm10_5 << 25) | (imm4_1 << 8) | (imm11 << 7);  /* Appends the immediate segments */
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_utype(uint8_t opcode, FILE* output, char** args, size_t num_args) {
  ...
  /* generate instruction */
  instruction = instruction | opcode;                                     /*Shifts 0 bits for opcode and puts it into instruction*/
  instruction = instruction | (rd << 7);                                  /*Shifts 7 bits for rt and puts it into instruction*/
  instruction = instruction | immediate << 12;                            /*Shifts 12 bits for immediate and puts it into instruction*/
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_ujtype(uint8_t opcode, FILE* output, char** args, size_t num_args, 
  ...
  imm11 = (I & 0x800) >> 11;
  imm10_1 = (I & 0x7fe) >> 1;
  instruction = instruction | (imm20 << 31) | (imm10_1 << 21) | (imm11 << 20) | (imm19_12 << 12);  /* Appends the immediate segments */
  write_inst_hex(output, instruction);
  return 0;
}
