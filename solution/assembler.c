...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  while (fgets(buf, BUF_SIZE, input))                                 /*Reads each line*/
  {
      ...
      {
          error = add_if_label(lineCount, next, byteOffset, symtbl);  /*Defines error*/
          if (error != 0)
          {
              next = strtok(NULL, IGNORE_CHARS);                      /*If error != 0, update next*/
          }
          if (next != NULL)                                           /*If next is not NULL*/
          {
              ...
              while (next != NULL)
              {
                  ...
                  {
                      pass = 0;
                      raise_extra_argument_error(lineCount, next);/*Raises such error if i exceeds max args*/
              ...
              if (flag) {continue;}
              label = write_pass_one(output, name, args, i);          /*Get written line count*/
              if (label == 0 && pass)
              {
                  raise_instruction_error(lineCount, name, args, i);  /*Raise instruction error if label == 0*/
                  error = -1;                                         /*Updates error by -1*/
              }
              byteOffset += 4 * label;                                /*Increase instruction memory byte offset by 4 * label*/
          }
          if (error == -1 || flag == 1)
          {
              result = -1;
...
/* First, read the next line into a buffer. */
    ...
    while (fgets(buf, BUF_SIZE, input))                                 /*Reads every line*/
    {
        ...
        if (translate_inst(output, args_name, args, i, byteOffset, symtbl, reltbl) == -1) {
            raise_instruction_error(lineCount, args_name, args, i);     /*If translation fails*/
            error = -1;                                                 /*Sets error to -1*/
        }
