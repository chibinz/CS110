...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, sizeof(buf), input) != NULL){
        ...
        /* deal with label */
        switch(add_if_label(lineNumber, token, byteOffset, symtbl)){
            case 0:
        ...
        insWritten = write_pass_one(output, name, args, num_args);
        if(insWritten == 0){
            /* If an error occurs,
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, sizeof(buf), input) != NULL){
        ...
        if(translate_inst(output, name, args, num_args, byteOffset, symtbl) == -1){
            /* If an error occurs,
               the instruction will not be written and you should call
               raise_instruction_error().  */
            raise_instruction_error(lineNumber, name, args, num_args);
