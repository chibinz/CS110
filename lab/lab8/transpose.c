    ...
    for (int i = 0; i <= blocks - 1; i++)
        for (int j = 0; j <= blocks - 1; j++)
            for (int a = 0; a < blocksize; a++)
                ...
                {
                    dst[n * (i * blocksize + a) + (j * blocksize + b)] =
                        src[i * blocksize + a + n * (j * blocksize + b)];
                }
    for (int i = 0; i <= blocks - 1; i++)
        for (int a = 0; a < blocksize; a++)
            for (int b = 0; b < n - blocks * blocksize; b++)
            {
                dst[n * (i * blocksize + a) + ((blocks)*blocksize + b)] =
                    src[i * blocksize + a + n * ((blocks)*blocksize + b)];
            }
    for (int j = 0; j <= blocks - 1; j++)
        for (int a = 0; a < n - blocks * blocksize; a++)
            for (int b = 0; b < blocksize; b++)
            {
                dst[n * ((blocks)*blocksize + a) + (j * blocksize + b)] =
                    src[(blocks)*blocksize + a + n * (j * blocksize + b)];
            }
    for (int a = 0; a < n - blocks * blocksize; a++)
        for (int b = 0; b < n - blocks * blocksize; b++)
        {
            dst[n * ((blocks)*blocksize + a) + ((blocks)*blocksize + b)] =
                src[(blocks)*blocksize + a + n * ((blocks)*blocksize + b)];
        }
}
